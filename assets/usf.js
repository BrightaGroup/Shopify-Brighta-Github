/* USF file - DO NOT MODIFY THIS FILE. THIS FILE IS REGULARLY CHANGED BY USF APP AND **ANY DIRECT CHANGES WILL BE LOST**. Use our in-app customization if you need to update CSS and JS code. Auto modified at: 2/26/2026 9:12:45 PM*/
/* Begin custom theme code */
// define templates for the Ella-Halothemes-6.0.0 theme
...
var _usfProductCard = `
<div class="product-item" :class="{'enable_background_button_card': _usfGlobalSettings.enable_background_button_card}" :data-product-id="product.id" :data-json-product="dataJson">
    <div class="card">
...
<div class="product-item" :data-product-id="product.id" :data-json-product="dataJson">
    <div class="card">
        <div :class="'card-product ' + (product.images.length ? '' : 'card--text-only card--soft')">
            <div class="card-product__wrapper usf-sr-product__image-container">
                <usf-badges v-if="_usfGlobalSettings.show_badge" :sale="hasDiscount" :soldOut="isSoldOut" :product="product" :badgeClass="'card__badge'" :salePercent="salePercent" :loc="loc" :pIndex="pIndex"></usf-badges>

                <a class="card-media media--loading-effect" :class="['card-media--' + _usf_media_size,{'media--hover-effect': hoverImage && _usfGlobalSettings.show_image_swap}]" :href="productUrl" @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave" :style="cardMediaStyle">
                     `+_usfImageProductCard+`
                    <!-- product image extra -->
                    <usf-plugin name="searchResultsProductPreview" :data="pluginData"></usf-plugin>
                    <usf-plugin name="searchResultsProductCart" :data="pluginData"></usf-plugin>
...
<div class="product-item" :class="{'sold-out': isSoldOut,'has-notify': isSoldOut && _usfGlobalSettings.show_notify_form}" :data-product-id="product.id" :data-json-product="dataJson">
    <div class="card">
        <div :class="'card-product ' + (product.images.length ? '' : 'card--text-only card--soft')">
            <div class="card-product__wrapper usf-sr-product__image-container">
                <usf-badges v-if="_usfGlobalSettings.show_badge" :sale="hasDiscount" :soldOut="isSoldOut" :product="product" :badgeClass="'card__badge'" :salePercent="salePercent" :loc="loc" :pIndex="pIndex"></usf-badges>
...
<div class="product-item" :class="{'disable_product_card_border': _usfGlobalSettings.disable_product_card_border}" :data-product-id="product.id" :data-json-product="dataJson">
    <div class="card">
        <div :class="'card-product ' + (product.images.length ? '' : 'card--text-only card--soft')">
            <div class="card-product__wrapper usf-sr-product__image-container">
                <usf-badges v-if="_usfGlobalSettings.show_badge" :sale="hasDiscount" :soldOut="isSoldOut" :product="product" :badgeClass="'card__badge'" :salePercent="salePercent" :loc="loc" :pIndex="pIndex"></usf-badges>
        ...
        <div class="card-information">
            <div class="card-information__wrapper" :class="['text-' + _usfGlobalSettings.product_content_text_align]">
                ...
                <a class="card-title link-underline" :class="{'card-title-ellipsis': _usfGlobalSettings.product_title_line_text != 'full', 'card-title-change': product.selectedVariantId}" :href="productUrl" :data-product-title="product.title" :data-product-url="'/products/' + product.urlName">
                    <span class="text">
                        <span v-html="product.title"></span>
                    </span>
                </a>
    ...
    <div class="card">
        <div :class="'card-product ' + (product.images.length ? '' : 'card--text-only card--soft')">
            <div class="card-product__wrapper usf-sr-product__image-container">
                ...
                <div v-if="_usfGlobalSettings.show_action" class="card-action" :class="{'has-compare': _usfGlobalSettings.show_compare && _usfGlobalSettings.product_compare_type == 'default'}">
                    <div v-if="_usfGlobalSettings.show_quick_view && _usfGlobalSettings.product_quick_view_type == 'default'" class="card-quickview above-button">
                        <button class="quickview-button button" :title="loc.quickView" :data-product-id="product.id" data-open-quick-view-popup :data-product-handle="product.urlName">
                            <span class="visually-hidden" v-html="loc.quickView"></span>
                            <span v-html="loc.quickView"></span>
                        </button>
...
function _usfOnAddToCartSuccess(rs, formSelector) {
    if(window.after_add_to_cart.type){
        switch (window.after_add_to_cart.type) {
            case 'cart':
                _usfHalo.redirectTo(window.routes.cart);
                break;
            case 'quick_cart':
                if (window.quick_cart.show) {
                    Shopify.getCart((cart) => {
                        if (window.quick_cart.type == 'popup') {
                            ...
                            // _usfHalo.updateDropdownCart(cart);
                        } else {
                            $('body').addClass('cart-sidebar-show');
                            _usfHalo.updateSidebarCart(cart);
                        }
            ...
            case 'popup_cart_1':
                Shopify.getCart((cart) => {
                    _usfHalo.updatePopupCart(cart, 1, variantId);
                    _usfHalo.updateSidebarCart(cart);
    ...
    }else if(window.routes && window.routes.cart)
        window.location = window.routes.cart;
}
